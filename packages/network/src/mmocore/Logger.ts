...
export default class Logger implements ILogger {
  ...
  constructor(ctx: string, level?: LogLevel) {
    ...
    if (level) {
      ...
      // Cast avoids depending on Vite's ambient types in this package; when
      // bundled by Vite, "import.meta.env.VITE_*" is still statically replaced.
      const envLevel = (import.meta as any)?.env?.VITE_L2JSC_LOG_LEVEL;
      if (envLevel) {
        this._logLevel = parseInt(envLevel, 10) as LogLevel;
      }
    }
  }
