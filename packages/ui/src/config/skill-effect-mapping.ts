...
/**
 * Reactive read, not baked onto the entity: UiStore.skillEffectFields loads
 * asynchronously (see UiStore.loadSkillEffectFields()), same treatment as
 * npc-race-mapping.ts's getNpcRace(). AbnormalStatusUpdate never sends a
 * buff's category, only id/level/remaining -- see @lineage2js/network's
...
export function getSkillEffectCategory(skillId: number): EffectCategory | undefined {
  const fields = rootStore.ui.skillEffectFields[skillId];
  if (!fields) {
    return undefined;
  }
