...
- Movement without animation -- the rig (utils/skeleton/humanoid-rig.ts) takes a HumanoidPose of bone rotations, so a walk cycle is a matter of driving that per frame; nothing reads it yet beyond the static idle pose
- Basic 3D models for mobs -- CreatureModel (components/core/scene/creature-model.component.tsx) is the extension point: mobs and summons still tint the old CharacterModel capsule by NpcRace (players and NPCs now render on the humanoid rig instead, see utils/skeleton/humanoid-rig.ts -- deliberately not extended to mobs, a humanoid skeleton is the wrong shape for a wolf). Swap that branch's return for real per-archetype geometry when it exists, no caller changes needed
