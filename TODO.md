...
- More character animation states -- idle/walk/run/death are converted and driven from the creature snapshot (CreatureModel's animationFor); the rigs also ship attacks, casting, sitting and social poses, but nothing in the client knows when a creature is doing any of those yet. Adding one is a line in convert-unity-models.ts's CLIPS plus whatever state drives it
- Basic 3D models for mobs -- CreatureModel (components/core/scene/creature-model.component.tsx) is the extension point: mobs and summons still tint the old CharacterModel capsule by NpcRace, since the converted bodies are all humanoid and that's the wrong shape for a wolf. The Unity project does carry monster models (Data/Animations/LineageMonsters*, and unlike the player rigs their animations are embedded in the FBX rather than split into .anim files), so this is mostly a matter of extending convert-unity-models.ts and giving CharacterBody a per-archetype model url
- Equipped armor/weapon visuals -- needed for both players and NPCs/mobs, rendered through CreatureModel/PlayerModel once there's something to render them with:
  ...
  - Still missing: the display-id -> armor-piece lookup. The pipeline itself now exists (assets-server/scripts/convert-unity-models.ts), and the source project ships the armor sets alongside the bare bodies it currently converts -- same six pieces per set, numbered `m001`, `m002`, ... instead of the `m000` it takes today -- so the missing half is the table mapping a paperdoll display id to a set number. WorldCreatureSnapshot doesn't carry the ids yet either (add if/when a rendering plan exists, no point exposing unused data to the UI store).
...
- Geodata integration
- Character textures -- the converted bodies are untextured and tinted flat per skin/outfit/hair slot (see character-body.component.tsx); the source project has the texture set, so this is a converter change plus dropping the tint
- Clan window
