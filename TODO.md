...
- **Shops/Trade**: MultiSellList (0xd0) -- no parser for the reply or a window to show it yet. Not yet confirmed against a multisell list that actually exists: the two tried live (blacksmith Aios' `multisell 1001`/`1004`, the Dualsword Craft Stamp recipes) both turned out to be a lineage2ts content gap, not a protocol round trip -- `multisell_items` (the datapack.database table) has entry ids 1001 and 1004, but no `multisell_groups` row references either as a list id, so `MultisellCache.separateAndSend` hits its `if (!container) return` and sends nothing at all -- same silent-exit shape lineage2ts uses throughout (a handler that has nothing to say just returns, no error, no reply). Confirmed the real client is equally silent against the same server, so this specific pair isn't this client's bug -- next time this comes up, check `multisell_groups`/`multisell_items` for the id first, before assuming it's ours to fix, PrivateStoreManageListSell (0xa0), PrivateStoreManageListBuy (0xbd), PrivateStoreListBuy (0xbe), ShopPreviewList (0xf5), ShopPreviewInfo (0xf6), RecipeShopSellList (0xdf), RecipeShopItemInfo (0xe0), SellListSeed (0xe9), BuyList (0xfe/0xb7), ExBuySellList (0xfe/0xb7)
