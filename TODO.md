...
- Implement the Assist action (client-side, no server packet found) -- Actions.ASSIST (id 6) has no RequestActionUse case (confirmed against HighFive and Interlude) and no dedicated packet either, unlike Attack/Pick Up/etc. The only lead is the incoming Attack broadcast (AttackMutator's "Attacked" event, currently unconsumed) -- tracking each creature's last-seen attack target from those broadcasts could let the client infer "my party member's current target" and hit() it, similar to how pickUpNearestItem() works, but this is unconfirmed against a real client and needs verification before implementing
